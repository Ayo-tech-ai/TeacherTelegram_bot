...
MAX_WORDS = 400 MAX_QUESTIONS = 5 MIN_CHUNK_WORDS = 50

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text( "👋 Hello! I'm your AI Question Generator Bot.\n\n" "📄 Send me a short passage (max 400 words), and I’ll generate up to 5 comprehension questions based on it.\n\n" "Use the format: \n\n" "/ask [number_of_questions]\n[Your passage here]" )

async def ask(update: Update, context: ContextTypes.DEFAULT_TYPE): try: if len(context.args) < 1: await update.message.reply_text("❗Please specify the number of questions (e.g., /ask 3 followed by your passage).") return
...
num_questions = int(context.args[0])
    ...
    # Handle leftover words in last chunk
...
if name == 'main': TELEGRAM_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN") if not TELEGRAM_TOKEN: raise ValueError("Please set the TELEGRAM_BOT_TOKEN environment variable.")
